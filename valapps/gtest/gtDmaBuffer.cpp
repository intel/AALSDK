...
TEST_P(dma_buffer_f, allocate_hold)
{
    ...
    if (size < 5 * afu_test::constants::MB)
    {
        ...
        ASSERT_NO_THROW(buffer = afu->allocate_buffer(size)) << "buffer allocation failed" << endl;
    ...
    {
        mmio_buffer::ptr_t buffer;
        ASSERT_THROW(buffer = afu->allocate_buffer(size), buffer_allocate_error) << "buffer allocate successful when expecting to throw and error" << endl;
    }

}

INSTANTIATE_TEST_CASE_P(sw_dma_01, 
                        dma_buffer_f,
                        ::testing::Combine(
                            ::testing::Range(1, 6), // size (in MB)
