#######################################################################
# gen_afu_env.py : Generate AFU paths, include directories as a
# Makefile snippet for ASE builds
#
# Author: Rahul R Sharma <rahul.r.sharma@intel.com>
#
# Run: python gen_afu_env.py <AFU sources Parent Directory>
# 
...
def show_help():
    ...
    print("python gen_afu_env.py -bus <cci|spl> <dir 1> [dir 2] ... [dir n]")
    print("")
...
#    tolowarg_list = map(lambda x:x.lower(),arg_list)
    tolowarg_list = []
    for arg in arg_list:
    ...
    # Select bus type
    # if "--bus" in arg_list: 
    #     bus_opt_position = tolowarg_list.index("--bus")
    #     bus_type = arg_list[bus_opt_position + 1]
    # elif "-b" in arg_list: 
    #     bus_opt_position = tolowarg_list.index("-b")
    #     bus_type = arg_list[bus_opt_position + 1]
    # else:
    #     bus_type = "cci"
    # if (bus_type not in ["cci", "spl"]):
    #     print ("ILLEGAL : Bus Type ", bus_type, " was supplied, DEFAULT to 'cci'") 
    #     ASE_INTF = "cci"
    # else:
    #     ASE_INTF = bus_type
    # print("Protocol : ", ASE_INTF)
    #######################################################################
    # Select Tool brand
    if "--tool" in arg_list: 
...
fd.write("#####################################################################\n")
fd.write("#                                                                   #\n")
fd.write("# Intel QuickAssist AFU Simulation Environment (w/ AALSDK 4.1)      #\n")
fd.write("# This file was generated by AALSDK/ase/scripts/gen_afu_env.py      #\n")
fd.write("#                                                                   #\n")
fd.write("#####################################################################\n")
fd.write("\n\n")
...
if len(ase_hw_files_str) == 0:
    ...
    print("ERROR: gen_afu_env.py script will now exit.\n")
    ase_functions.end_red_fontcolor()
    sys.exit(0)
