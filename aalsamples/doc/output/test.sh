...
timeout 10s Hello_AAL/SW/helloaal >OUTPUT.txt 2>&1
...
timeout 10s Hello_CCI_NLB/SW/helloCCInlb >OUTPUT.txt 2>&1
temp=`grep -c "Error" OUTPUT.txt`
	if [ $temp -ne 0 ];
	then
	temp=0;
...
timeout 10s Hello_SPL_LB/SW/helloSPLlb >OUTPUT.txt 2>&1
temp=`grep -c "Error" OUTPUT.txt`
	if [ $temp -ne 0 ];
	then
	sed -n 29,40p doc/output/err.txt >>errtemp.txt
