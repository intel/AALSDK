...
TEST(RRMBrokerService, aal0804)
{
   // When librrmbroker is opened via OSServiceModuleOpen(), the value queried
   // by the standard entry point command C matches the value created by the
   // configure.ac macro M, where {C, M} are {AAL_SVC_CMD_VER_STR, RRMBROKER_VERSION},
   // {AAL_SVC_CMD_VER_CURRENT, RRMBROKER_VERSION_CURRENT},
   // {AAL_SVC_CMD_VER_REVISION, RRMBROKER_VERSION_REVISION},
   // {AAL_SVC_CMD_VER_AGE, RRMBROKER_VERSION_AGE}.
   ...
   std::string path(GlobalTestConfig::GetInstance().Vpath() +
                    std::string("/aas/RRMBrokerService/")   +
                    lt_objdir);
...
# error Implement path resolution for Windows.
   ...
   OSServiceModuleInit(&mod, "librrmbroker");

   if ( 0 != OSServiceModuleOpen(&mod) ) {
#if   defined( __AAL_LINUX__ )
      UnRequireLD_LIBRARY_PATH(path.c_str());
#endif // OS
      FAIL() << "couldn't open librrmbroker";
   }
